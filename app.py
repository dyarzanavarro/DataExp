...
def load_data(path=DATA_FILE):
    ...
    df["AnzZuzuWir"] = pd.to_numeric(
        df["AnzZuzuWir"], errors="coerce").fillna(0).astype(int)
...
def main():
    st.set_page_config(
        page_title="Moving to Zurich — Data Explorer", layout="wide")
    ...
    selected_origins = st.sidebar.multiselect(
        "Origin", origin_opts, default=origin_opts)

    age_opts = sorted(df["age_group"].unique())
    selected_ages = st.sidebar.multiselect(
        "Age groups", age_opts, default=age_opts)
    ...
    ts = (
        filtered.groupby(["date"])["AnzZuzuWir"].sum(
    ...
    with col1:
        fig = px.line(ts, x="date", y="AnzZuzuWir",
                      title="Monthly arrivals (count)")
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        # top quarters table
        top_quarters = (
            filtered.groupby("quarter")["AnzZuzuWir"].sum(
            ).reset_index().sort_values("AnzZuzuWir", ascending=False)
        )
        st.write("Top quarters")
        st.dataframe(top_quarters.head(10).style.format(
            {"AnzZuzuWir": "{:,}"}), height=400)
    ...
    with tab1:
        grouped = (
            filtered.groupby(["year", "origin"])[
                "AnzZuzuWir"].sum().reset_index()
        )
        fig2 = px.area(grouped, x="year", y="AnzZuzuWir",
                       color="origin", title="Arrivals by origin over years")
        st.plotly_chart(fig2, use_container_width=True)

    with tab2:
        grouped2 = (
            filtered.groupby(["year", "age_group", "sex"])[
                "AnzZuzuWir"].sum().reset_index()
        )
        fig3 = px.bar(grouped2, x="year", y="AnzZuzuWir", color="age_group",
                      barmode="stack", facet_col="sex", title="Arrivals by age group and sex")
        st.plotly_chart(fig3, use_container_width=True)

    with tab3:
        kreis_top = (
            filtered.groupby(["kreis"])["AnzZuzuWir"].sum(
            ).reset_index().sort_values("AnzZuzuWir", ascending=False)
        )
        st.write("Top districts (Kreis)")
        st.dataframe(kreis_top.head(20).style.format(
            {"AnzZuzuWir": "{:,}"}), height=400)
